...
class ConfigApp(admin.MConfigHandler):
  ...
  def handleList(self, confInfo):
    confDict = self.readConf("tanium")
    if None != confDict:
      for stanza, settings in confDict.items():
        for key, val in settings.items():
            if key in ['taniumhost'] and val in [None, '']:
                val = ''
  ...
  def handleEdit(self, confInfo):
    ...
    if self.callerArgs.data['taniumhost'][0] in [None, '']:
      self.callerArgs.data['taniumhost'][0] = ''
...
write them to the [setupentity] stanza
    ...
    self.writeConf('tanium', 'taniumserver', self.callerArgs.data)
