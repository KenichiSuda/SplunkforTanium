...
class TaniumQuestion:
	...
	def loop_poll_tanium (self,saved,timeout=10):
		"""
	...
	def get_xml_results_from_tanium (self,saved):
		"""
	...
	def ask_tanium_a_question (self,saved,timeout):

		#self.last_id = self.send_request_to_tanium (sensors)
		status = self.loop_poll_tanium (saved,timeout)
		if status == "Complete":
			return self.get_xml_results_from_tanium (saved)
		else:
			return "Timeout"
